    ...
    <div className={`${card} rounded-2xl overflow-hidden shadow-lg hover:scale-105 transition`}>
      ...
      <div className="p-6">
        ...
        <Link
          ...
          } w-full py-3 rounded-xl text-center block font-semibold`}
        >
          View Details
        </Link>
      </div>
...
export default function HomePage() {
  ...
  return (
    ...
    >
      <main className={`${bg} transition-colors duration-500 min-h-screen relative`}>
        ...
        <section className="max-w-7xl mx-auto px-6 py-24">
          ...
          {loading ? (
            <div className="grid md:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-80 rounded-2xl animate-pulse bg-gray-700/50" />
              ))}
            </div>
          ) : (
            <div className="grid md:grid-cols-3 gap-8">
        ...
        <section className="max-w-7xl mx-auto px-6 py-24 grid md:grid-cols-3 gap-8 text-center">
          <div className={`${card} rounded-3xl p-10`}>
            ...
            <p className="text-gray-400 mt-2">Professional Providers</p>
          </div>
          <div className={`${card} rounded-3xl p-10`}>
            <CheckCircle className="mx-auto mb-4" size={40} />
            <h3 className="text-4xl font-bold">1200+</h3>
            <p className="text-gray-400 mt-2">Bookings Completed</p>
          </div>
          <div className={`${card} rounded-3xl p-10`}>
