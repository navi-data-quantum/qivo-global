...
const login = asyncHandler(async (req, res) => {
  ...
  if (!user.is_verified)
    throw new AppError(await tService.t("EMAIL_NOT_VERIFIED", user.language), 403);

  if (user.lock_until && user.lock_until > Date.now())
    throw new AppError(await tService.t("ACCOUNT_LOCKED", user.language), 423);
  ...
  if (!match) {
    ...
    if (attempts >= MAX_ATTEMPTS)
      await lockAccount(user.id, Date.now() + LOCK_TIME);
...
const refreshToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new AppError(await tService.t("INVALID_EXPIRED_TOKEN", "en"), 401);
  }
  ...
  if (decoded.tokenVersion !== user.token_version)
    throw new AppError(await tService.t("TOKEN_REVOKED", user.language), 403);
...
const logout = asyncHandler(async (req, res) => {
  ...
  res.json({
    success: true,
    message: await tService.t("LOGGED_OUT_EVERYWHERE", "en")
  });
});
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  res.json({
    success: true,
    message: await tService.t("RESET_PASSWORD_SENT_IF_EXISTS", "en")
  });
});
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  res.json({
    success: true,
    message: await tService.t("PASSWORD_UPDATED", user.language)
  });
});
...
const setLanguage = asyncHandler(async (req, res) => {
  ...
  if (!language)
    throw new AppError(await tService.t("LANGUAGE_REQUIRED", "en"), 400);

  await updateUserLanguage(req.user.id, language);

  res.json({
    success: true,
    message: await tService.t("LANGUAGE_UPDATED", language),
    language
  });
});
